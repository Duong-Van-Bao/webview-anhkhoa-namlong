  ...
  export default {
    ...
    methods: {
      ...
      addModal() {
        ...
        this.typeForm = 'ADD'
        this.apiForm = '/api/CollectTypes/AddCollectType'
        setTimeout(() => {
          this.$bvModal.show(this.nameForm)
        }, timer)
      },
      ...
      editModal(items) {
        this.getDataByID(items[0].CollectTypeID)
        setTimeout(() => {
          ...
          this.typeForm = 'EDIT'
          this.apiForm = '/api/CollectTypes/EditCollectType'
          this.idForm = items[0].CollectTypeID
          setTimeout(() => {
            this.$bvModal.show(this.nameForm)
          }, timer)
        }, timer)
      ...
      deleteModal(items) {
        this.$bvModal.msgBoxConfirm(this.$t('form.question') + items[0].CollectTypeName + '?', {
          ...
          .then(value => {
            if (value === true) {
              this.handleSubmitDelete(items[0].CollectTypeID);
            }
          })
          .catch(err => {
            console.log(err);
      ...
      async handleSubmitDelete(id) {
        ...
        methods.post("/api/CollectTypes/DeleteCollectType", body).then(val => {
          if (val !== null && val !== '' && val !== undefined && val.StatusCode === 200 && val.Success === 1) {
      ...
      statusEdit(data) {
        let body = {
          ID: data.CollectTypeID,
          IsActive: handling.convertBooleanToBit(data.IsActive),
          UserIDCurent: this.userIDCurrent,
          MenuIDCurent: this.objectData.menuIDCurrent
        }
        methods.post("/api/CollectTypes/ChangeStatusCollectType", body).then(val => {
          if (val !== null && val !== '' && val !== undefined && val.StatusCode === 200 && val.Success === 1) {
            setTimeout(() => {
              this.getData()
              this.makeToast('success', this.$t('toast.success').toUpperCase(), val.Message)
      ...
      getDataByID(id) {
        ...
        methods.post("/api/CollectTypes/GetCollectTypeById", body).then(val => {
          if (val !== null && val !== '' && val !== undefined && val.StatusCode === 200 && val.Success === 1) {
      ...
      getData() {
        methods.post("/api/CollectTypes/GetListCollectType", {}).then(val => {
          if (val !== null && val !== '' && val !== undefined && val.StatusCode === 200 && val.Success === 1) {
    ...
    async created() {
      ...
      await this.getColumn('collecttypes')
      await this.getKeyTable()
      await this.getData()
      await this.getFormAddShow('collecttypes')
    },
    computed: {
      fields: function () {
