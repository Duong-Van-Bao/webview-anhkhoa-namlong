  ...
  export default {
    ...
    methods: {
      ...
      addModal() {
        ...
        this.typeForm = 'ADD'
        this.apiForm = '/api/Indicators/AddIndicators'
        setTimeout(() => {
          this.$bvModal.show(this.nameForm)
        }, timer)
      },
      ...
      editModal(items) {
        this.getDataByID(items[0].IndicatorID)
        setTimeout(() => {
          ...
          this.typeForm = 'EDIT'
          this.apiForm = '/api/Indicators/EditIndicators'
          this.idForm = items[0].IndicatorID
          setTimeout(() => {
            this.$bvModal.show(this.nameForm)
          }, timer)
        }, timer)
      ...
      deleteModal(items) {
        this.$bvModal.msgBoxConfirm(this.$t('form.question') + items[0].IndicatorName + '?', {
          ...
          .then(value => {
            if (value === true) {
              this.handleSubmitDelete(items[0].IndicatorID);
            }
          })
          .catch(err => {
            console.log(err);
      ...
      async handleSubmitDelete(id) {
        ...
        methods.post("/api/Indicators/DeleteIndicators", body).then(val => {
          if (val !== null && val !== '' && val !== undefined && val.StatusCode === 200 && val.Success === 1) {
      ...
      statusEdit(data) {
        ...
        methods.post("/api/Indicators/ChangeStatusIndicators", body).then(val => {
          if (val !== null && val !== '' && val !== undefined && val.StatusCode === 200 && val.Success === 1) {
            setTimeout(() => {
              this.getData()
              this.makeToast('success', this.$t('toast.success').toUpperCase(), val.Message)
      ...
      getDataByID(id) {
        ...
        methods.post("/api/Indicators/GetListIndicatorsById", body).then(val => {
          if (val !== null && val !== '' && val !== undefined && val.StatusCode === 200 && val.Success === 1) {
    ...
    async created() {
      ...
      await this.getColumn('indicators')
      await this.getKeyTable()
      await this.getData()
      await this.getFormAddShow('indicators')
    },
    computed: {
      fields: function () {
