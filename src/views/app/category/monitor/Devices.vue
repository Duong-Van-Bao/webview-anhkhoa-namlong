    ...
    methods: {
      ...
      editModal(items) {
        ...
        setTimeout(() => {
          ...
          this.idForm = items[0].DeviceInfoID
          setTimeout(() => {
            this.$bvModal.show(this.nameForm)
          }, timer)
        }, timer)
      ...
      deleteModal(items) {
        this.$bvModal.msgBoxConfirm(this.$t('form.question') + items[0].DeviceInfoName + '?', {
      ...
      async handleSubmitDelete(id) {
        ...
        methods.post("/api/DeviceInfos/DeleteDeviceInfo", body).then(val => {
          if (val !== null && val !== '' && val !== undefined && val.StatusCode === 200 && val.Success === 1) {
      ...
      statusEdit(data) {
        ...
        methods.post("/api/DeviceInfos/ChangeStatusDeviceInfo", body).then(val => {
          if (val !== null && val !== '' && val !== undefined && val.StatusCode === 200 && val.Success === 1) {
            setTimeout(() => {
              this.getData()
              this.makeToast('success', this.$t('toast.success').toUpperCase(), val.Message)
      ...
      getDataByID(id) {
        ...
        methods.post("/api/DeviceInfos/GetListDeviceInfosById", body).then(val => {
          if (val !== null && val !== '' && val !== undefined && val.StatusCode === 200 && val.Success === 1) {
      ...
      getData() {
        methods.post("/api/DeviceInfos/GetListDeviceInfo", {}).then(val => {
          if (val !== null && val !== '' && val !== undefined && val.StatusCode === 200 && val.Success === 1) {
