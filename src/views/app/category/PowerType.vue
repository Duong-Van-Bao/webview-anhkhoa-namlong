  ...
  export default {
    ...
    methods: {
      ...
      addModal() {
        ...
        this.typeForm = 'ADD'
        this.apiForm = '/api/PowerTypes/AddPowerType'
        setTimeout(() => {
          this.$bvModal.show(this.nameForm)
        }, timer)
      },
      ...
      editModal(items) {
        this.getDataByID(items[0].PowerTypeID)
        setTimeout(() => {
          ...
          this.typeForm = 'EDIT'
          this.apiForm = '/api/PowerTypes/EditPowerType'
          this.idForm = items[0].PowerTypeID
          setTimeout(() => {
            this.$bvModal.show(this.nameForm)
          }, timer)
        }, timer)
      ...
      deleteModal(items) {
        this.$bvModal.msgBoxConfirm(this.$t('form.question') + items[0].PowerTypeName + '?', {
          ...
          .then(value => {
            if (value === true) {
              this.handleSubmitDelete(items[0].PowerTypeID);
            }
          })
          .catch(err => {
            console.log(err);
      ...
      async handleSubmitDelete(id) {
        ...
        methods.post("/api/PowerTypes/DeletePowerType", body).then(val => {
          if (val !== null && val !== '' && val !== undefined && val.StatusCode === 200 && val.Success === 1) {
      ...
      statusEdit(data) {
        ...
        methods.post("/api/PowerTypes/ChangeStatusPowerType", body).then(val => {
          if (val !== null && val !== '' && val !== undefined && val.StatusCode === 200 && val.Success === 1) {
            setTimeout(() => {
              this.getData()
              this.makeToast('success', this.$t('toast.success').toUpperCase(), val.Message)
      ...
      getDataByID(id) {
        ...
        methods.post("/api/PowerTypes/GetPowerTypeById", body).then(val => {
          if (val !== null && val !== '' && val !== undefined && val.StatusCode === 200 && val.Success === 1) {
