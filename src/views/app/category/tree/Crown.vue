...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      editModal(items) {
        this.getDataByID(items[0].CrownID)
        setTimeout(() => {
      ...
      deleteModal(items) {
        this.$bvModal.msgBoxConfirm(this.$t('form.question') + items[0].CrownName + '?', {
          ...
          .then(value => {
            if (value === true) {
              this.handleSubmitDelete(items[0].CrownID);
            }
          })
          .catch(err => {
            console.log(err);
      ...
      async handleSubmitDelete(id) {
        ...
        methods.post("/api/Crowns/Delete", body).then(val => {
          if (val !== null && val !== '' && val !== undefined && val.StatusCode === 200 && val.Success === 1) {
      ...
      statusEdit(data) {
        ...
        methods.post("/api/Crowns/ChangeStatus", body).then(val => {
          if (val !== null && val !== '' && val !== undefined && val.StatusCode === 200 && val.Success === 1) {
            setTimeout(() => {
              this.getData()
              this.makeToast('success', this.$t('toast.success').toUpperCase(), val.Message)
      ...
      getDataByID(id) {
        ...
        methods.post("/api/Crowns/GetCrownById", body).then(val => {
          if (val !== null && val !== '' && val !== undefined && val.StatusCode === 200 && val.Success === 1) {
