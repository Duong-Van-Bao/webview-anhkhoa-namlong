  ...
  export default {
    ...
    methods: {
      ...
      addModal() {
        ...
        this.typeForm = 'ADD'
        this.apiForm = '/api/TreeGroups/Add'
        setTimeout(() => {
          this.$bvModal.show(this.nameForm)
        }, timer)
      },
      ...
      editModal(items) {
        this.getDataByID(items[0].TreeGroupID)
        setTimeout(() => {
          ...
          this.typeForm = 'EDIT'
          this.apiForm = '/api/TreeGroups/Edit'
          this.idForm = items[0].TreeGroupID
          setTimeout(() => {
            this.$bvModal.show(this.nameForm)
          }, timer)
        }, timer)
      ...
      deleteModal(items) {
        this.$bvModal.msgBoxConfirm(this.$t('form.question') + items[0].TreeGroupName + '?', {
          ...
          .then(value => {
            if (value === true) {
              this.handleSubmitDelete(items[0].TreeGroupID);
            }
          })
          .catch(err => {
            console.log(err);
      ...
      async handleSubmitDelete(id) {
        ...
        methods.post("/api/TreeGroups/Delete", body).then(val => {
          if (val !== null && val !== '' && val !== undefined && val.StatusCode === 200 && val.Success === 1) {
      ...
      statusEdit(data) {
        ...
        methods.post("/api/TreeGroups/ChangeStatus", body).then(val => {
          if (val !== null && val !== '' && val !== undefined && val.StatusCode === 200 && val.Success === 1) {
            setTimeout(() => {
              this.getData()
              this.makeToast('success', this.$t('toast.success').toUpperCase(), val.Message)
      ...
      getDataByID(id) {
        ...
        methods.post("/api/TreeGroups/GetTreeGroupById", body).then(val => {
          if (val !== null && val !== '' && val !== undefined && val.StatusCode === 200 && val.Success === 1) {
    ...
    async created() {
      ...
      await this.getColumn('treegroups')
      await this.getKeyTable()
      await this.getData()
      await this.getFormAddShow('treegroups')
    },
    computed: {
      fields: function () {
