...
<script>
  ...
  import CustomFormTree from '../../../components/Form/CustomFormFunction'
  ...
  export default {
    ...
    methods: {
      ...
      deleteModal(items) {
        this.$bvModal.msgBoxConfirm(this.$t('form.question') + items[0].OID + '?', {
      ...
      async handleSubmitDelete(id) {
        ...
        methods.post("/api/collect/DelCollect", body).then(val => {
          if (val !== null && val !== '' && val !== undefined && val.StatusCode === 200 && val.Success === 1) {
      ...
      statusEdit(data) {
        let body = {
          OID: data.OID,
          IsClose: handling.convertBooleanToBit(data.IsClose),
          UserIDCurent: this.userIDCurrent,
          MenuIDCurent: this.objectData.menuIDCurrent
        }
        methods.post("/api/collect/EditIsCloseCollection", body).then(val => {
          if (val !== null && val !== '' && val !== undefined && val.StatusCode === 200 && val.Success === 1) {
            setTimeout(() => {
              this.getData()
              this.makeToast('success', this.$t('toast.success').toUpperCase(), val.Message)
            ...
            if (val.Result.length > 0) {
              ...
              for (let i = 0; i < val.Result.length; i++) {
                if (val.Result[i].IsActive === 1) {
                  let obj = {
                    text: val.Result[i].CollectTypeName,
                    value: val.Result[i].CollectTypeID,
                  }
      ...
      getDataByID(id) {
        ...
        methods.post("/api/collect/GetCollectByOID", body).then(val => {
          if (val !== null && val !== '' && val !== undefined && val.StatusCode === 200 && val.Success === 1) {
      ...
      getData() {
        methods.post("/api/collect/GetListCollect", {}).then(val => {
          if (val !== null && val !== '' && val !== undefined && val.StatusCode === 200 && val.Success === 1) {
