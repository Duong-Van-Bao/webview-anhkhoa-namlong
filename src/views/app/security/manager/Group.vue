        ...
        <div class="top-right-button-container">
          <b-button-group>
            <b-dropdown right :text="$t('dropdown.action')" variant="primary">
              <b-dropdown-item @click="addModal()" v-if="objectData.accessWrite === true"><i class="fad fa-plus"></i>&emsp;{{$t('dropdown.add')}}
              </b-dropdown-item>
              ...
              <b-dropdown-item @click="editModal(selectedItems)"
                ...
                class="fad fa-edit"></i>&emsp;{{$t('dropdown.edit')}}
              </b-dropdown-item>
              <b-dropdown-item v-else disabled><i class="fad fa-edit"></i>&emsp;{{$t('dropdown.edit')}}
              </b-dropdown-item>
              <b-dropdown-item @click="deleteModal(selectedItems)"
                               v-if="selectedItems.length === 1 && objectData.accessWrite === true"><i
                class="fad fa-trash-alt"></i>&emsp;{{$t('dropdown.delete')}}
              </b-dropdown-item>
              <b-dropdown-item v-else disabled><i class="fad fa-trash-alt"></i>&emsp;{{$t('dropdown.delete')}}
              </b-dropdown-item>
            </b-dropdown>
          </b-button-group>
        </div>
  ...
  export default {
    ...
    methods: {
      infoModal(items) {
        ...
        this.titleForm = this.$t('form.title-info')
        this.typeForm = 'INFO'
        setTimeout(() => {
      ...
      addModal() {
        ...
        this.titleForm = this.$t('form.title-add')
        this.typeForm = 'ADD'
        this.apiForm = '/api/groupuser/AddGroupUser'
        setTimeout(() => {
          this.$bvModal.show(this.nameForm)
        }, timer)
      },
      ...
      editModal(items) {
        ...
        setTimeout(() => {
          ...
          this.titleForm = this.$t('form.title-edit')
          this.typeForm = 'EDIT'
          this.apiForm = '/api/groupuser/EditGroupUser'
          this.idForm = items[0].GroupID
          setTimeout(() => {
            this.$bvModal.show(this.nameForm)
          }, timer)
        }, timer)
      ...
      deleteModal(items) {
        this.$bvModal.msgBoxConfirm(this.$t('form.question') + items[0].GroupName + '?', {
      ...
      async handleSubmitDelete(id) {
        ...
        methods.post("/api/groupuser/DelGroupUser", body).then(val => {
          if (val !== null && val !== '' && val !== undefined && val.StatusCode === 200 && val.Success === 1) {
      ...
      statusEdit(data) {
        ...
        methods.post("/api/groupuser/EditStatusGroupUser", body).then(val => {
          if (val !== null && val !== '' && val !== undefined && val.StatusCode === 200 && val.Success === 1) {
            setTimeout(() => {
              this.getData()
              this.makeToast('success', this.$t('toast.success').toUpperCase(), val.Message)
      ...
      getDataByID(id) {
        ...
        methods.post("/api/groupuser/GetGroupUserByID", body).then(val => {
          if (val !== null && val !== '' && val !== undefined && val.StatusCode === 200 && val.Success === 1) {
    ...
    async created() {
      ...
      await this.getColumn('groupusers')
      await this.getKeyTable()
      await this.getData()
      await this.getFormAddShow('groupusers')
    },
    computed: {
      fields: function () {
