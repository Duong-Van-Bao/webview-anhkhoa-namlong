  ...
  export default {
    ...
    methods: {
      ...
      addModal() {
        ...
        this.typeForm = 'ADD'
        this.apiForm = '/api/DataTypes/AddDataType'
        setTimeout(() => {
          this.$bvModal.show(this.nameForm)
        }, timer)
      },
      ...
      editModal(items) {
        this.getDataByID(items[0].DataTypeID)
        setTimeout(() => {
          ...
          this.typeForm = 'EDIT'
          this.apiForm = '/api/DataTypes/EditDataType'
          this.idForm = items[0].DataTypeID
          setTimeout(() => {
            this.$bvModal.show(this.nameForm)
          }, timer)
        }, timer)
      ...
      deleteModal(items) {
        this.$bvModal.msgBoxConfirm(this.$t('form.question') + items[0].DataTypeName + '?', {
          ...
          .then(value => {
            if (value === true) {
              this.handleSubmitDelete(items[0].DataTypeID);
            }
          })
          .catch(err => {
            console.log(err);
      ...
      async handleSubmitDelete(id) {
        ...
        methods.post("/api/DataTypes/DeleteDataType", body).then(val => {
          if (val !== null && val !== '' && val !== undefined && val.StatusCode === 200 && val.Success === 1) {
      ...
      statusEdit(data) {
        ...
        methods.post("/api/DataTypes/ChangeStatusDatatype", body).then(val => {
          if (val !== null && val !== '' && val !== undefined && val.StatusCode === 200 && val.Success === 1) {
            setTimeout(() => {
              this.getData()
              this.makeToast('success', this.$t('toast.success').toUpperCase(), val.Message)
      ...
      getDataByID(id) {
        ...
        methods.post("/api/DataTypes/GetDataTypeById", body).then(val => {
          if (val !== null && val !== '' && val !== undefined && val.StatusCode === 200 && val.Success === 1) {
    ...
    async created() {
      ...
      await this.getColumn('datatypes')
      await this.getKeyTable()
      await this.getData()
      await this.getFormAddShow('datatypes')
    },
    computed: {
      fields: function () {
