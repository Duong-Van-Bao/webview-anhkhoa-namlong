...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      editModal(items) {
        this.getDataByID(items[0].IndicatorWarningID)
        setTimeout(() => {
          ...
          this.idForm = items[0].IndicatorWarningID
          setTimeout(() => {
            this.$bvModal.show(this.nameForm)
          }, timer)
        }, timer)
      ...
      deleteModal(items) {
        this.$bvModal.msgBoxConfirm(this.$t('form.question') + items[0].IndicatorWarningName + '?', {
      ...
      async handleSubmitDelete(id) {
        ...
        methods.post("/api/IndicatorWarnings/DeleteIndicatorWarning", body).then(val => {
          if (val !== null && val !== '' && val !== undefined && val.StatusCode === 200 && val.Success === 1) {
      ...
      statusEdit(data) {
        ...
        methods.post("/api/IndicatorWarnings/ChangeStatusIndicatorWarning", body).then(val => {
          if (val !== null && val !== '' && val !== undefined && val.StatusCode === 200 && val.Success === 1) {
            setTimeout(() => {
              this.getData()
              this.makeToast('success', this.$t('toast.success').toUpperCase(), val.Message)
      ...
      getOptionIndicator(){
        methods.post("/api/Indicators/GetListIndicators", {}).then(val => {
          if (val !== null && val !== '' && val !== undefined && val.StatusCode === 200 && val.Success === 1) {
            if (val.Result.length > 0) {
              ...
              for (let i = 0; i < val.Result.length; i++) {
                if (val.Result[i].IsActive === 1) {
                  let obj = {
                    text: val.Result[i].IndicatorName,
                    value: val.Result[i].IndicatorCode,
                  }
          ...
          if (val !== null && val !== '' && val !== undefined && val.StatusCode === 200 && val.Success === 1) {
            if (val.Result.length > 0) {
              ...
              for (let i = 0; i < val.Result.length; i++) {
                if (val.Result[i].IsActive === 1) {
                  let obj = {
                    text: val.Result[i].LevelWarningName,
                    value: val.Result[i].LevelWarningID,
                  }
      ...
      getDataByID(id) {
        ...
        methods.post("/api/IndicatorWarnings/GetIndicatorWarningById", body).then(val => {
          if (val !== null && val !== '' && val !== undefined && val.StatusCode === 200 && val.Success === 1) {
      ...
      getData() {
        methods.post("/api/IndicatorWarnings/GetListIndicatorWarning", {}).then(val => {
          if (val !== null && val !== '' && val !== undefined && val.StatusCode === 200 && val.Success === 1) {
    ...
    async created() {
      ...
      await this.getColumn('indicatorwarnings')
      await this.getKeyTable()
      await this.getData()
      await this.getFormAddShow('indicatorwarnings')
